...
const AssociateDirectory = () => {
  ...
  const filteredAssociates = associates.filter(associate => {
    const matchesSearch = associate.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         ...
                         associate.certifications.some(cert => cert.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <div className="flex items-center space-x-1 mb-3">
            ...
            <span className="text-sm font-medium text-gray-900">{associate.rating}</span>
            <span className="text-sm text-gray-500">({associate.completedJobs} jobs)</span>
          </div>
        </div>
        
        <div className="text-right">
          <div className="text-lg font-semibold text-gray-900">${associate.hourlyRate}/hr</div>
        </div>
      </div>
  ...
  const AssociateDetailsModal = ({ associate, onClose }: { associate: Associate; onClose: () => void }) => (
            ...
            <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-gray-900">{associate.location.address}</p>
                </div>
                <div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-gray-900">${associate.hourlyRate}/hour</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Rating</label>
                  <div className="flex items-center">
                    <Star className="h-4 w-4 text-yellow-400 fill-current mr-1" />
                    <span className="text-gray-900">{associate.rating}/5.0</span>
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Completed Jobs</label>
                  <p className="text-gray-900">{associate.completedJobs}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Joined</label>
                  <p className="text-gray-900">{associate.joinedAt.toLocaleDateString()}</p>
                </div>
              </div>
            </div>
          </div>
